...
const UstozlarDavomati = ({ teachers, setTeachers }) => {
  const L = useAttendanceLogic(teachers, setTeachers); 
  const [modal, setModal] = useState(false);
  const [editingTeacher, setEditingTeacher] = useState(null); // Состояние для редактируемого юзера
  ...
  // Закрыть модалку и сбросить редактирование
    ...
    <S.Container>
      <S.Toolbar>
        ...
        <div style={{display:'flex', gap:'8px'}}>
          <button onClick={exportExcel} style={{padding:'10px', cursor:'pointer', background:'white', border:'1px solid #ddd', borderRadius:'8px', display: 'flex', alignItems: 'center', gap: '5px'}}>
            📥 Excel
          </button>
          <S.PrimaryBtn onClick={() => { setEditingTeacher(null); setModal(true); }}>➕ Добавить</S.PrimaryBtn>
        </div>
      </S.Toolbar>
      ...
      <S.MainGrid>
        ...
        <aside>
          <S.StatBox color="#3B82F6">
            <div style={{fontSize:'14px', color:'#64748B'}}>Всего учителей</div>
            <div className="val">{L.stats.total}</div>
          </S.StatBox>
          
          <S.StatBox color="#22C55E" p={L.stats.pKelgan}>
            <div style={{fontSize:'14px', color:'#64748B'}}>Пришли ({L.stats.pKelgan}%)</div>
            <div className="val">{L.stats.kelgan}</div>
            <div className="bar"><div className="fill" style={{width: `${L.stats.pKelgan}%`}}/></div>
          </S.StatBox>
          
          <S.StatBox color="#EF4444" p={L.stats.pKelmagan}>
            <div style={{fontSize:'14px', color:'#64748B'}}>Не пришли ({L.stats.pKelmagan}%)</div>
            <div className="val">{L.stats.kelmagan}</div>
            <div className="bar"><div className="fill" style={{width: `${L.stats.pKelmagan}%`}}/></div>
          </S.StatBox>
        </aside>
      </S.MainGrid>
      ...
      {modal && (
        <AddTeacherModal 
          ...
          onSave={(data) => { 
            if (editingTeacher) {
              L.updateTeacher(data); // Если было редактирование
            } else {
              L.addTeacher(data); // Если новый учитель
            }
            handleCloseModal(); 
          }} 
        />
