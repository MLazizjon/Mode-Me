...
const Guruh2 = () => {
  ...
  const [students, setStudents] = useState(() => {
    const saved = localStorage.getItem("guruh2_data");
    return saved ? JSON.parse(saved) : [];
  });
  const [modal, setModal] = useState({ show: false, student: null });

  useEffect(() => {
    localStorage.setItem("guruh2_data", JSON.stringify(students));
  }, [students]);
              ...
              {students.map((s, idx) => (
                <tr key={s.id}>
                  ...
                  <td style={{ textAlign: 'left' }}>
                    ...
                    <Link to={`/profil/guruh2_data/${s.id}`} style={{ textDecoration: 'none', color: '#007bff', fontWeight: 'bold' }}>
                      {s.name}
                    </Link>
                  </td>
                  <td>
